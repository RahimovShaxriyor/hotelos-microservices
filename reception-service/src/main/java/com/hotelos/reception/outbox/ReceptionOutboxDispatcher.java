...
@Component
public class ReceptionOutboxDispatcher {
    private static final Logger log = LoggerFactory.getLogger(ReceptionOutboxDispatcher.class);
    ...
    public ReceptionOutboxDispatcher(
    ...
    public boolean dispatchOne() {
        Optional<ReceptionOutboxEventEntity> candidate = outboxRepository.findNextEligibleEventForUpdate();
        if (candidate.isEmpty()) {
            return false;
        }

        ReceptionOutboxEventEntity event = candidate.get();
    ...
    private void handleFailure(ReceptionOutboxEventEntity event, String reason) {
