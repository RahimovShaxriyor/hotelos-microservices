...
@Component
public class RoomServiceOutboxDispatcher {
    private static final Logger log = LoggerFactory.getLogger(RoomServiceOutboxDispatcher.class);
    ...
    public RoomServiceOutboxDispatcher(
    ...
    public boolean dispatchOne() {
        Optional<RoomServiceOutboxEventEntity> candidate = outboxRepository.findNextEligibleEventForUpdate();
        if (candidate.isEmpty()) {
            return false;
        }

        RoomServiceOutboxEventEntity event = candidate.get();
    ...
    private void handleFailure(RoomServiceOutboxEventEntity event, String reason) {
